...
- **Average month (Phase 11):** the top card. Shows what a normal month costs, over the last 3, 6 or 12 **completed** months — the month in progress is deliberately never averaged in, because a part-month drags the mean down and makes the figure creep upward all month. It is shown alongside instead, as "this month so far". The switch defaults to 6 months and remembers the choice in `localStorage` under `trim:avgWindow`. All three windows arrive in one `/api/analytics` response, so switching never refetches. When history is shorter than the window the card says so honestly — "last 8 completed" for a 12m window with 8 months of data — rather than dividing by 12.
  - The card follows the page's **incl. / excl. special** chip (below); it no longer owns a toggle of its own. While excluding, its caption ends "· excl. special" so the headline figure always says which basis it is on.
...
- **incl. / excl. special chip (page-wide, 5 Sep 2026):** one chip beside the "Analytics" heading that flips **every figure on the page** between counting special expenses and leaving them out — the average, this month / last month / change, the chart's expense line, the top-category bars and every row of the monthly history. While excluding, the chip turns amber and a line under the heading says "Special expenses are out of every figure on this page", so a filtered page can never be mistaken for an unfiltered one.
  - It replaces a chip that lived *inside* the average card and moved only that card. The bug that fixed: flipping it left last month, the chart, the category bars and the history table still counting one-offs while the average did not — two different measures side by side on one screen, with nothing on the page saying so.
  - Shown only when the special-expenses pref is on **and** some month on the page has special spend. With the pref off the flags are dormant, exactly as Phase 9.2 promises: no chip, no note, no star column, every figure all-in — even if a previous "excl." choice is still remembered.
  - Owned by `pages/Analytics.jsx` and passed down, the way Dashboard owns its hero's. Persisted under `trim:avgIncludeSpecial` — the old key name is kept deliberately, so an existing choice survives the change; still deliberately separate from the hero's `trim:heroIncludeSpecial`, since that toggles one figure and this toggles a page.
  - No refetch: `/api/analytics` serves both bases at once. `mom` carries `thisMonthSpecial`, `lastMonthSpecial` and `deltaPctExclSpecial` beside the all-in figures (a percentage has one definition, on the server, so it can never disagree with the two figures beside it), and `topCategoriesExclSpecial` is its **own** top five — taking special spend out can change *which* five categories are on top, which a client-side filter of the incl. list cannot do. Mirrored into `scripts/devMock.js`; covered by 3 route tests × 3 encryption phases.
...
- **Monthly history (Task 9.4):** a full history table below the chart, fetched as 24 months of data (the chart still only plots the last 6). One row per month, newest first — Spent, Income, Net, and (when `user_stats.special_expenses_enabled` is on and at least one month has flagged special spend) a Special column with a star marker. Spent and Net follow the page's incl./excl. chip; the Special column keeps showing the slice either way, so while excluding it reads as "what was taken out". Which months are listed is measured on the all-in figures, so the list never grows or shrinks as the chip is flipped. Months before the user's first transaction are trimmed from the list; the current month is labelled "so far". Tapping a row opens `/transactions?month=YYYY-MM`, which deep-links Transactions straight to that month, fetching it directly from the API rather than relying on the page's default 200-row recent window.
...
- **Cost ceiling that actually holds (23 Sep 2026):** 20 user turns per rolling hour, counted in Postgres as `ask_messages` rows with `role='user'` in the trailing hour. The previous `express-rate-limit` guard used the default in-memory store, but production is a **Vercel serverless function**, not a long-lived process — every instance had its own counter, so the documented cap was really 20 × however many instances were warm, and a caller who waited for recycling met no limit at all. Postgres is shared by construction and survives recycling. The check runs **before** the user-message insert, so a rejected turn leaves no stray row and cannot inflate the caller's own quota; the in-memory limiter stays as a cheap first line. Exceeding it returns 429 with "You've been chatting a lot — try again in a bit."
...
- **Who may hold an account: `ALLOWED_EMAILS` (23 Sep 2026).** Supabase signup is open and email confirmation is off, and the anon key is public in the client bundle by design — so a verified token proved only that *our project issued it*, never that the holder was invited. Anyone could self-register and stand on all 14 authenticated mounts. `requireAuth` now checks the token's email against a comma-separated allowlist after the signature/issuer/audience/expiry checks, and answers **403** rather than 401: the credential is genuine, the account simply is not invited, so retrying with a fresh token will never help. Matching is trimmed and lower-cased on both sides.
  - It **fails closed at boot** — an unset or empty list makes the server refuse to start, the same way a missing `SUPABASE_URL` does, because an absent allowlist must never read as "everyone is welcome". The operational cost is accepted deliberately: `ALLOWED_EMAILS` must be set in the `trim-api` environment *before* this code is deployed, and must name **every** real user plus the test account, or those users get a 403 they cannot fix themselves.
  - Covered by `server/test/auth.test.js` — the first tests in the suite ever to load the real middleware, since every route suite mounts behind a stub that injects `req.user`.
