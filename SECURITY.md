...
| `ALLOWED_EMAILS` | **Server only** (`server/.env` + the Vercel project's env) | Comma-separated emails permitted to hold a Trim account. Verifying a Supabase token proves our project issued it, **not** that the holder was invited — signup is open and email confirmation is off (below), so this list is the only thing keeping strangers off the authenticated API and off `ANTHROPIC_API_KEY`. `requireAuth` fails closed at boot if it is unset: the server refuses to start rather than admitting everyone. Must list every real user before deploying, or they are locked out. |
...
- [x] Email confirmation: **OFF everywhere** — load-bearing consequence: `ALLOWED_EMAILS` is what actually restricts accounts; do not remove it while this is off. (decided 2026-07-14 — the built-in sender's ~2 emails/hour cap plus spam-foldering made signups look broken at friends scale). Revisit if Trim is ever shared beyond friends; the Signup page already handles the confirm-email flow if it's re-enabled.
