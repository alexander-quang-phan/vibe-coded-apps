...
│   │   ├── pages/           # Dashboard, Transactions, Budgets, Analytics, SavingsGoals, Settings, Login, Signup
...
│   ├── routes/              # me, categories, transactions, dashboard, budgets, analytics, goals
...
- **RLS enabled on every table** (categories, transactions, budgets, savings_goals, savings_contributions, user_stats). Policies are `USING (auth.uid() = user_id)`. Service-role bypasses RLS but we still scope every query by `req.user.id` — defence in depth.
